...
public class HuaweiAlgorithm extends CrackAlgorithm {
	...
	protected void setPatterns() {
		...
		/*
		 * TODO: Add this...
		 * // Los que tienen un simple comentario están verificados, los que
		 * tienen DOBLE comentario
		 * // NO FUNCIONAN, pero como la gente se queja...pues hala...
		 * matcher_hawei[2] =
		 * Pattern.compile("(00:25:68:[0-9A-Fa-f:]{8})").matcher(mBSSID);//Visto
		 * en HG520 español
		 * matcher_hawei[3] =
		 * Pattern.compile("(00:25:9E:[0-9A-Fa-f:]{8})").matcher(mBSSID);//Visto
		 * en routerkeygen
		 * matcher_hawei[4] =
		 * Pattern.compile("(04:C0:6F:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[5] =
		 * Pattern.compile("(08:19:A6:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[6] =
		 * Pattern.compile("(0C:37:DC:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[7] =
		 * Pattern.compile("(10:C6:1F:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[8] =
		 * Pattern.compile("(1C:1D:67:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[9] =
		 * Pattern.compile("(20:2B:C1:[0-9A-Fa-f:]{8})").matcher(mBSSID);//Visto
		 * en routerkeygen
		 * matcher_hawei[10] =
		 * Pattern.compile("(20:F3:A3:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[11] =
		 * Pattern.compile("(24:DB:AC:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[12] =
		 * Pattern.compile("(28:5F:DB:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[13] =
		 * Pattern.compile("(28:6E:D4:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[14] =
		 * Pattern.compile("(30:87:30:[0-9A-Fa-f:]{8})").matcher
		 * (mBSSID);/////HG265
		 * matcher_hawei[15] =
		 * Pattern.compile("(40:4D:8E:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[16] =
		 * Pattern.compile("(4C:54:99:[0-9A-Fa-f:]{8})").matcher(mBSSID);//Visto
		 * en Telmex, HG530 (pero en vodafone no funciona!!!!!)
		 * matcher_hawei[17] =
		 * Pattern.compile("(4C:1F:CC:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[18] =
		 * Pattern.compile("(54:A5:1B:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[19] =
		 * Pattern.compile("(54:89:98:[0-9A-Fa-f:]{8})").matcher(mBSSID);
		 * matcher_hawei[20] =
		 * Pattern.compile("(64:16:F0:[0-9A-Fa-f:]{8})").matcher(mBSSID);//Visto
		 * a varias VodafoneXXXX
		 * matcher_hawei[21] =
		 * Pattern.compile("(00:E0:FC:[0-9A-Fa-f:]{8})").matcher
		 * (mBSSID);//////Huawei wa1003A
		 * matcher_hawei[22] =
		 * Pattern.compile("(00:18:82:[0-9A-Fa-f:]{8})").matcher
		 * (mBSSID);/////Por lo visto es el que tiene el Huawei U8100/8150, y
		 * otros routers
		 * //matcher_hawei[23] =
		 * Pattern.compile("(30:87:30:[0-9A-Fa-f:]{8})").matcher
		 * (mBSSID);/////Visto en algunas Vodafone, pero tb en Orange
		 * matcher_hawei[23] =
		 * Pattern.compile("(24:DB:AC:[0-9A-Fa-f:]{8})").matcher(mBSSID);/////
		 * matcher_hawei[24] =
		 * Pattern.compile("(00:0F:E2:[0-9A-Fa-f:]{8})").matcher(mBSSID);/////
		 * matcher_hawei[25] =
		 * Pattern.compile("(28:6E:D4:[0-9A-Fa-f:]{8})").matcher(mBSSID);/////
		 * matcher_hawei[26] =
		 * Pattern.compile("(00:11:F5:[0-9A-Fa-f:]{8})").matcher(mBSSID);/////
		 * matcher_hawei[27] =
		 * Pattern.compile("(F4:C7:14:[0-9A-Fa-f:]{8})").matcher(mBSSID);/////Es
		 * de Orange, no comercializa ese router
		 * //matcher_hawei[29] =
		 * Pattern.compile("(78:1D:BA:[0-9A-Fa-f:]{8})").matcher
		 * (mBSSID);/////VodafoneInternetMovil
		 * matcher_hawei[28] =
		 * Pattern.compile("(5C:4C:A9:[0-9A-Fa-f:]{8})").matcher(mBSSID);/////Es
		 * de Orange, no comercializa ese router
		 */
	}
